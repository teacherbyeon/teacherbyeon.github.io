...
function clearParticipantAuth(className = state.className) {
  if (!className) return;
  const idMap = getParticipantIdMap();
  const secretMap = getParticipantSecretMap();
  delete idMap[className];
  delete secretMap[className];
  localStorage.setItem(participantIdMapKey, JSON.stringify(idMap));
  localStorage.setItem(participantSecretMapKey, JSON.stringify(secretMap));
}
...
function createSlotCard(slot) {
  ...
  if (isTeacherAuthorized()) {
    ...
    zoomBtn.disabled = !slot.thumbUrl;
    zoomBtn.onclick = (e) => {
      e.stopPropagation();
    ...
    analyzeBtn.textContent = slot.aiStatus === 'running' ? '분석중...' : '분석';
    analyzeBtn.disabled = !slot.thumbUrl || slot.aiStatus === 'running';
    analyzeBtn.onclick = (e) => {
      e.stopPropagation();
      socket?.emit('ai:analyze-slot', { className: state.className, slotId: slot.id, teacherToken: getTeacherToken() }, (ack = {}) => {
        if (!ack.ok) alert(ack.message || '개별 AI 분석 실패');
      });
    };
    ...
    removeBtn.disabled = !slot.thumbUrl;
    removeBtn.onclick = (e) => {
      e.stopPropagation();
    ...
    actions.append(zoomBtn, analyzeBtn, removeBtn);
...
function renderManagePage() {
  ...
  (state.participants || []).forEach((p) => {
    ...
    kickBtn.onclick = () => {
      if (!confirm(`${p.nickname || '해당 학생'}을(를) 강퇴할까요?`)) return;
      socket?.emit('participant:kick', { className: state.className, participantId: p.participantId, teacherToken: getTeacherToken() }, (ack = {}) => {
        if (!ack.ok) return alert(ack.message || '강퇴 실패');
        alert('학생을 강퇴했습니다.');
      });
    };
    kickTd.appendChild(kickBtn);
    tr.appendChild(kickTd);
    tbody.appendChild(tr);
  });

  const filter = byId('aiResultFilter');
  const list = byId('aiResultList');
  if (!list) return;
  const currentFilter = filter?.value || 'all';
  const slots = (board?.slots || []).filter((s) => {
    if (currentFilter === 'all') return true;
    if (currentFilter === 'done' || currentFilter === 'running' || currentFilter === 'error') return (s.aiStatus || 'idle') === currentFilter;
    return (s.aiCandidateTag || 'none') === currentFilter;
  });
  list.innerHTML = '';
  if (!slots.length) {
    list.innerHTML = '<p class="hint">표시할 AI 결과가 없습니다.</p>';
  } else {
    slots.forEach((s) => {
      const card = document.createElement('div');
      card.className = 'ai-result-card';
      card.innerHTML = `
        <strong>${s.nickname || '익명'} (${s.classroom || '-'} / ${s.studentNo || '-'})</strong>
        <p><b>상태:</b> ${s.aiStatus || 'idle'} / <b>후보:</b> ${s.aiCandidateTag || 'none'} / <b>신뢰도:</b> ${s.aiConfidence || 'low'}</p>
        <p><b>교사용 요약:</b> ${s.aiTeacherSummary || '-'}</p>
        <p><b>학생 피드백:</b> ${s.aiFeedbackForStudent || '-'}</p>
        <p><b>이유:</b> ${Array.isArray(s.aiReasons) ? s.aiReasons.join(' · ') : '-'}</p>
        <p><b>분석 시각:</b> ${s.aiAnalyzedAt ? new Date(s.aiAnalyzedAt).toLocaleString() : '-'}</p>
      `;
      list.appendChild(card);
    });
  }
  filter && (filter.onchange = renderManagePage);
}
...
function initBoardPage() {
  ...
  socket?.on('participant:kicked', (payload = {}) => {
    alert(payload.message || '교사에 의해 수업에서 퇴장되었습니다. 다시 입장해 주세요.');
    clearParticipantAuth(state.className);
    go('./join.html');
  });
